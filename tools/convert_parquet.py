"""
This tool converts Parquet files to JSONL (JSON Lines) format.

Purpose:
- Takes Parquet files matching a specified pattern and combines them into a single JSONL file
- Handles complex data types like numpy arrays, timestamps, and binary data by converting them to JSON-serializable formats
- Particularly useful for converting dataset files from Parquet (efficient columnar storage) to JSONL (easier to process line by line)
- Outputs information about the dataset structure after conversion

Usage:
- Set the parquet_pattern variable to point to your Parquet files using a glob pattern
- Run the script to generate a corresponding JSONL file in the same location
"""
