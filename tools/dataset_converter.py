"""
This tool converts various dataset formats into a unified conversation format for training language models.
It includes several rewriting functions for different dataset types:

- alpaca_gpt4: Converts instruction-input-output format into conversations
- codefeedback: Converts query-answer pairs into conversations
- multitask: Converts system-prompt-chosen triples into conversations
- dolly: Converts instruction-context-response format into conversations
- gpt_roleplay: Converts role-play dialogues into conversations
- random_samples: Converts raw text into a single-turn conversation
- sharegpt: Converts ShareGPT format, optionally dropping system messages
- magpie_reasoning: Converts instruction-response pairs into conversations
- custom: Updates data from an older conversation format

Each converter transforms its specific format into a unified conversation structure with:
- init: System message/context (if any)
- conversations: List of turns with "from" (human/gpt) and "value" (text content)
- source: Dataset name for tracking data origin
- tags: Optional metadata tags ("completion", ...)

The script uses parallel processing to efficiently handle large datasets and
optionally sorts conversations by length.

Usage:
    Set the input_file_path, rewriting_function, and other parameters at the bottom 
    of the script, then run to convert your dataset.
"""
